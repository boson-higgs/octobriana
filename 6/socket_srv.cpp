...
void clientfunc(int socket, int clientIndex, char playerSymbol){
    int l_read;
    while(1){
        char buffer[128] = {0};
        ...
        g_glb_data->gameBoard[atoi(col)][rowIndex] = playerSymbol;
        write(socket, g_glb_data->gameBoard, strlen((char*)g_glb_data->gameBoard));
...
int main( int t_narg, char **t_args )
{
    ...
    while ( 1 )
    {
        ...
        clientCounter++;
	    char playerSymbol = (clientCounter % 2 ==0) ? 'X' : 'O';

        if(fork()==0){

            clientfunc(newSocket,clientCounter, playerSymbol);
        }
    } // while ( 1 )
    close(newSocket);
    return 0;

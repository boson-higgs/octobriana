...
int client_proccess(int socket, int listen)
{
    pollfd l_read_poll[ 2 ];

    l_read_poll[ 0 ].fd = STDIN_FILENO;
    l_read_poll[ 0 ].events = POLLIN;
    l_read_poll[ 1 ].fd = listen;
	...
	if ((pid = fork()) == 0)
	{
		int l_sock_listen = listen;
		int l_sock_client = socket;
	    ...
	    { // communication
	        ...
	        if ( l_read_poll[ 1 ].revents & POLLIN )
	        {
					...
					std::string com = strtok(l_buf, " ");
					std::cout << "strtok: " << com;
					std::string par = s.substr(com.length() + 1);
					printf("\n\tcommand: %s\n\tparams: %s", com.c_str(), par.c_str());
	
					//char* l_param = new char[par.length() + 1];
					//strcpy(l_param, par.c_str());
					//printf("test %s\n", l_param);
					int s_t;
					sem_getvalue(g_mutex, &s_t);
					printf("\tsem: %d\n", s_t);
					sem_wait(g_mutex);
					...
					if (l_child == 0)
					{
						...
						char* l_p = new char[par.length()-2];
						strncpy(l_p, par.c_str(), par.length()-2);
						//printf("\tlen: %d\n\n", strlen(l_p));
						//printf("\tl_p: '%s'\n\n", l_p);
						execlp( com.c_str(), com.c_str(), l_p, nullptr );
					...
					{ // parent
						log_msg( LOG_INFO, "Parent if." );
	
						char ex_res[2048];
						close(l_mypipe[1]);
						int l_ret = read( l_mypipe[0], ex_res, sizeof(ex_res));
						
						write ( l_sock_client, ex_res, l_ret);
						//printf("buff: %s\n\n\n", ex_res);				
...
{
	g_mutex = sem_open( "first", O_RDWR | O_CREAT, 0660, 0 );
    ...
    while ( 1 )
    {
		...
		client_proccess(l_sock_client, l_sock_listen);
    } // while ( 1 )

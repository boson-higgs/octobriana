...
void client_handler(int t_sock_client)
{
    char buffer[1024];
    int len;
    ...
    while((len = read(t_sock_client, buffer, sizeof(buffer) - 1)) > 0)
    {
        buffer[len] = '\0';
        printf("Accepted from client: %s\n", buffer);

		pid_t l_command_child = fork();
		if(l_command_child == -1)
		{
			log_msg( LOG_ERROR, "Unable to fork command_child");
			exit(EXIT_FAILURE);
		}

		if(l_command_child == 0)
		{ //child process
			close(l_pipe[0]); //zavrit cteci konec
			
			dup2(l_pipe[1], STDOUT_FILENO);
			close(l_pipe[1]);
			//musime rozdelit prikaz na slova pro execvp
			char *args[256];
			int arg_c = 0;
			char *token = strtok(buffer, " ");
			while(token != NULL && arg_c < 255)
			{
				args[arg_c++] = token;
				token = strtok(NULL, " ");
			}
			args[arg_c] = NULL; //posledni arguemtn musi byt null
			execvp(args[0], args);
			log_msg( LOG_ERROR, "execvp failed");
			//exit(EXIT_FAILURE);
			//system(buffer);
		}
		else
		{
			close(l_pipe[1]); //zavrit zapisovaci konec
			
			char command_output_buffer[1024];
			int output_ret_len = read(l_pipe[0], command_output_buffer, sizeof(command_output_buffer) - 1);
			command_output_buffer[output_ret_len] = '\0';

			write(t_sock_client, command_output_buffer, output_ret_len);
			close(l_pipe[0]);
		}

        if(strcmp(buffer, STR_CLOSE) == 0)
        {
...
int main( int t_narg, char **t_args )
{
	...
	log_msg(LOG_INFO, "SERVER IS RUNNING, WAITING FOR CLIENTS");
